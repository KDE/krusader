...
KIO::WorkerResult kio_krarcProtocol::put(const QUrl &url, int permissions, KIO::JobFlags flags)
{
    ...
    // pack the file
    KrLinecountingProcess proc;
    proc << putCmd << getPath(arcFile->url()) << tempFile;
...
KIO::WorkerResult kio_krarcProtocol::get(const QUrl &url, int tries)
{
    ...
    KrLinecountingProcess proc;
    if (extArcReady) {
        proc << getCmd << arcTempDir + QStringLiteral("contents.cpio") << QStringLiteral("./") + file;
    } else if (arcType == "arj" || arcType == "ace" || arcType == "7z") {
        proc << getCmd << getPath(arcFile->url(), QUrl::StripTrailingSlash) << file;
...
KIO::WorkerResult kio_krarcProtocol::del(QUrl const &url, bool isFile)
{
    ...
    KrLinecountingProcess proc;
    proc << delCmd << getPath(arcFile->url()) << file;
...
bool kio_krarcProtocol::initDirDict(const QUrl &url, bool forced)
{
    ...
    if (arcType == QStringLiteral("ace") && QFile(QStringLiteral("/dev/ptmx")).exists()) // Don't remove, unace crashes if missing!!!
        proc.setStandardInputFile(QStringLiteral("/dev/ptmx"));
...
KIO::WorkerResult kio_krarcProtocol::initArcParameters()
{
    ...
    if (arcType == "zip") {
        ...
        listCmd << fullPathName(QStringLiteral("ls")) << QStringLiteral("--literal"); // This is better than using `bzip2 -tv`
        ...
        listCmd << fullPathName(QStringLiteral("ls")) << QStringLiteral("--literal"); // This is better than using `lzma -tv`
        ...
        cmd = fullPathName("xz");
        listCmd << fullPathName(QStringLiteral("ls")) << QStringLiteral("--literal"); // This is better than using `xz -l`
...
void kio_krarcProtocol::check7zOutputForPassword(KProcess *proc, QByteArray &buf)
{
    ...
    for (QString &line : lines) {
        ...
        if ((line.startsWith(QStringLiteral("Enter password")) && line.endsWith(QStringLiteral(":"))) ||
