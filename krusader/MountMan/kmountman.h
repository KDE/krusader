...
public:
  ...
  inline bool operational(){ return Operational; } // check this 1st
  inline bool ready()  		 { return Ready; 	     } // poll on this until true
...
private:
inline void clearOutput() { delete outputBuffer; outputBuffer=0; }
inline QString getOutput(){ return QString((!outputBuffer ?
...
public:
  ...
  inline QString shortName(){ return Name.right(Name.length()-Name.find("/",1)-1); }
  inline QString type()     { return Type;                            }
  inline QString mntPoint() { return MntPoint;                        }
  inline long   totalBlks() { return TotalBlks;                       }
  inline long    freeBlks() { return (TotalBlks-UsedBlks);            }  
  inline long long  totalBytes() { return TotalBlks*1024;             }
  inline long long   usedBytes() { return UsedBlks*1024;              }
  inline long long   freeBytes() { return (TotalBlks-UsedBlks)*1024;  }
  //////////////////// insert a good round function here /////////////////
  int    usedPerct() { if (TotalBlks==0) return 0;
                       float res=((float)UsedBlks)/((float)TotalBlks)*100;
											 if ((res - (int)res) > 0.5) return (int)res + 1;
											 	else return (int)res;                  }
  inline bool     mounted() { return Mounted;                         }

  // set information
  inline void setName(QString n_)     { Name=n_;      }
  inline void setType(QString t_)     { Type=t_;      }
  inline void setMntPoint(QString m_) { MntPoint=m_;  }
  inline void setTotalBlks(long t_)   { TotalBlks=t_; }
  inline void setUsedBlks(long u_)    { UsedBlks=u_;  }
  inline void setMounted(bool m_)     { Mounted=m_;   }
