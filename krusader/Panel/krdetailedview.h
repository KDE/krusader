...
public:
    ...
    virtual inline KrViewItem *getFirst() {
        return dynamic_cast<KrViewItem*>(firstChild());
    }
    virtual inline KrViewItem *getLast() {
        return dynamic_cast<KrViewItem*>(lastChild());
    }
    virtual inline KrViewItem *getNext(KrViewItem *current) {
        return dynamic_cast<KrViewItem*>(dynamic_cast<K3ListViewItem*>(current) ->itemBelow());
    }
    virtual inline KrViewItem *getPrev(KrViewItem *current) {
        return dynamic_cast<KrViewItem*>(dynamic_cast<K3ListViewItem*>(current) ->itemAbove());
    }
    ...
    virtual inline KrViewItem *findItemByName(const QString &name) {
        return dynamic_cast<KrViewItem*>(findItem(name, 0));
    }
    ...
    virtual inline void saveSettings() {
        K3ListView::saveLayout(_config, nameInKConfig());
    }
    virtual inline void restoreSettings() {
        K3ListView::restoreLayout(_config, nameInKConfig());
    }
    virtual void redraw() {
        viewport()->show(); header()->show(); show();
    }
    ...
    static KrView* create(QWidget *parent, bool &left, KConfig *cfg) {
        return new KrDetailedView(parent, left, cfg);
    }

protected:
    virtual void setup();
    virtual KrViewProperties * createViewProperties() {
        return new KrDetailedViewProperties();
    }
...
protected slots:
    ...
    void transformCurrentChanged(Q3ListViewItem * item) {
