    ...
    m_parentAddress( parentAddress )
{
  ...
  // Changed by Jan Halasa - we want all the actions to be on the
  // bottom of the menu, no matter whether it is root menu or not.
...
void KBookmarkMenu::slotAddBookmark()
{
  ...
  // The user changed his mind and canceled the "Add bookmark" dialog.
  // changed by Jan Halasa for Krusader's needs
  if (url == 0) {
    return;
  }
  if (url.isEmpty())
...
void KBookmarkMenu::slotBookmarkSelected()
{
  ...
  m_pOwner->openBookmarkURL( bookmark.url() );  
}
