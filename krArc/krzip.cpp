...
int kdemain( int argc, char **argv ){
	KInstance instance( "kio_krzip" );

	if (argc != 4){
		kdWarning() << "Usage: kio_krzip  protocol domain-socket1 domain-socket2" << endl;
		exit(-1);
	}
    
	kio_krzipProtocol slave(argv[2], argv[3]);
	slave.dispatchLoop();
...
kio_krzipProtocol::kio_krzipProtocol(const QCString &pool_socket, const QCString &app_socket)
 : SlaveBase("kio_krzip", pool_socket, app_socket), archiveChanged(true), arcFile(0L){
...
/* ---------------------------------------------------------------------------------- */
kio_krzipProtocol::~kio_krzipProtocol(){
...
/* ---------------------------------------------------------------------------------- */
void kio_krzipProtocol::receivedData(KProcess*,char* buf,int len){
	QByteArray d(len);
	d.setRawData(buf,len);
	data(d);	
}

void kio_krzipProtocol::mkdir(const KURL& url,int permissions){
...
void kio_krzipProtocol::put(const KURL& url,int permissions,bool overwrite,bool resume){
...
void kio_krzipProtocol::get(const KURL& url ){
...
void kio_krzipProtocol::del(KURL const & url, bool isFile){
...
void kio_krzipProtocol::stat( const KURL & url ){
...
void kio_krzipProtocol::listDir(const KURL& url){
	QString path = url.path();
...
bool kio_krzipProtocol::setArcFile(const QString& path){
	archiveChanged = true;
...
bool kio_krzipProtocol::initDirDict(const KURL& url, bool forced){
...
QString kio_krzipProtocol::findArcDirectory(const KURL& url){
	QString path = url.path();
...
UDSEntry* kio_krzipProtocol::findFileEntry(const KURL& url){
  QString arcDir = findArcDirectory(url);
...
QString kio_krzipProtocol::nextWord(QString &s,char d) {
  s=s.stripWhiteSpace();
  int j=s.find(d,0);
...
mode_t kio_krzipProtocol::parsePermString(QString perm){
	mode_t mode=0;
  // file type
...
void kio_krzipProtocol::parseLine(int, QString line, QFile*){
  UDSEntryList* dir;
	UDSEntry entry;
	UDSAtom atom;
...
void kio_krzipProtocol::initArcParameters(){
	cmd = "zip";
